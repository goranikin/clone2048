...
const moveGridLeftLogic = (grid: gridType): moveResultType => {
  ...
  return { resultGrid: resultGrid, scoreIncrement: scoreIncrement.reduce((prev, curr) => prev + curr, 0) }
};
...
export const moveFunctions = {
  ...
  moveRight: (grid: gridType): moveResultType => {
    ...
    return { resultGrid: resultGrid.map((row) => row.reverse()), scoreIncrement };
  },
  ...
  moveDown: (grid: gridType): moveResultType => {
    const { resultGrid, scoreIncrement } = moveGridLeftLogic(transpose(grid).map((row) => row.reverse()));
    return { resultGrid: transpose(resultGrid.map((row) => row.reverse())), scoreIncrement };
  }
}